...
Because this is an exploratory analysis, I recommend trying out your code in an interactive environment such as jupyter notebook (https://ood.ris.wustl.edu/pun/sys/dashboard/).
Then, paste your completed code into mpsa_analysis.py and run it to complete the assignment.  
...
```bash
$ bsub -Is -q general-interactive -G compute-workshop -a 'docker(takinwe1/bio5488:0.0)' /bin/bash
...
---
## What to turn in
**Submit these three things to the submission folder!** `/storage1/fs1/workshops/Active/BIO5488/SP2026.L41.BIOL.5488.01/Assignments/A11/Submissions/Users/<User_name> `


**They are**
1. A completed `mpsa_analysis.py`
2. A `readme file` that contains the command to execture the script & the output of your file. Copy and paste the standard output from the terminal into the readme file and answer the questions.
3. The `output.txt` file. Run your script and redirect the output to a file. 
For example:

   ```
    python3 mpsa_analysis.py > output.txt
   ```
   or if the command line returns a CondaError when you attempt to activate the conda environment, you can hardcode the path to the tool:
   ```bash
   /opt/conda/envs/bio5488/bin/python3 mpsa_analysis.py > output.txt
   ```
